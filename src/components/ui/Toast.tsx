...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <Ctx.Provider value={api}>
      {children}
      <Viewport toasts={toasts} onDismiss={(id) => setToasts((prev) => prev.filter((t) => t.id !== id))} />
    </Ctx.Provider>
  );
}
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "pointer-events-auto w-[340px] glass-strong rounded-lg shadow-elev border-l-2 px-3.5 py-3 flex items-start gap-2.5",
